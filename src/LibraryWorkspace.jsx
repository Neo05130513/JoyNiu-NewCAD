...
import { createPartInstance, initialPartPosition, standardParts, validatePartDefinition } from './assemblyModel.js'
...
export default function LibraryWorkspace({ model, assemblyItems = [], setAssemblyItems, onOpenAssembly, onBackToModel, showToast }) {
  ...
  return <div className="aw-workspace">
    <header className="aw-heading"><div><span className="aw-eyebrow">STANDARD PARTS</span><h1>标准件库</h1><p>将有尺寸的独立实例加入「{model?.name || '当前零件'}」的装配。</p></div><div className="aw-actions"><button onClick={() => setCustomOpen((open) => !open)}>＋ 自定义零件</button><button className="aw-primary" onClick={onOpenAssembly}>查看装配（{assemblyItems.length + 1}） →</button></div></header>
