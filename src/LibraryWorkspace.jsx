...
import { createPartInstance, filterStandardParts, initialPartPosition, mainModelEnvelope, standardParts, validatePartDefinition } from './assemblyModel.js'
...
export default function LibraryWorkspace({ model, assemblyItems = [], setAssemblyItems, onOpenAssembly, onBackToModel, showToast }) {
  ...
  const hasMainModel = Boolean(envelope)
  const instanceCount = assemblyItems.length + (hasMainModel ? 1 : 0)
  const filtered = useMemo(() => filterStandardParts(query, group), [query, group])
  const insert = (part) => {
    ...
    const text = `已插入 ${instance.name} ${instance.spec}，可在标准件装配草稿中调整位置。`
    setNotice(text)
  ...
  return <div className="aw-workspace">
    <header className="aw-heading"><div><span className="aw-eyebrow">STANDARD PARTS</span><h1>标准件库</h1><p>将有尺寸的独立实例加入「{model?.name || '当前零件'}」的标准件装配草稿。</p></div><div className="aw-actions"><button onClick={() => setCustomOpen((open) => !open)}>＋ 自定义零件</button><button className="aw-primary" onClick={onOpenAssembly}>查看装配草稿（{instanceCount}） →</button></div></header>
    <div className="aw-library-layout"><section className="aw-card aw-library-main">
    ...
    </section><aside className="aw-card aw-library-side"><h2>当前装配草稿</h2>{hasMainModel ? <div className="aw-current-model"><span>◇</span><div><b>{model?.name || '当前零件'}</b><small>{envelope ? '当前模型 · 固定基准' : '主件尺寸待更新'}</small></div></div> : <p className="aw-muted">尚未创建主件，可先插入标准件并独立调整位置。</p>}<div className="aw-inserted-list">{assemblyItems.map((item, index) => <div key={item.id}><span>{String(index + 1).padStart(2, '0')}</span><div><b>{item.name}</b><small>{item.spec}</small></div></div>)}</div>{!assemblyItems.length && <p className="aw-muted">还没有插入标准件。每次插入都会创建一个可独立移动的实例。</p>}<button className="aw-primary aw-wide" onClick={onOpenAssembly}>进入装配草稿调整位置</button>{onBackToModel && <button className="aw-wide" onClick={onBackToModel}>返回 3D 建模</button>}<p className="aw-scope">规格、来源和位置随当前文件保存，切换文件可建立独立装配。</p></aside></div>
