...
function WorkbenchApp({ initialStore, account, workspace, suspended = false }) {
  ...
  return (
    <div className={`app-shell studio-shell text-size-${settings.textSize} ${navCollapsed ? 'studio--collapsed' : ''} ${activeMode === '特征编辑' ? 'studio--cad' : activeMode === '原生二维' ? 'studio--2d' : ''}`} hidden={suspended}>
      {!['特征编辑','原生二维'].includes(activeMode) && <StudioNavigation activeMode={activeMode} context={context} projectName={selectedProject} fileName={activeFile?.name}
