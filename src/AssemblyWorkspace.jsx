...
export default function AssemblyWorkspace({ model, assemblyItems = [], setAssemblyItems, onBackToModel, onOpenLibrary, showToast }) {
  ...
  return <div className="aw-workspace">
    <header className="aw-heading"><div><span className="aw-eyebrow">ASSEMBLY WORKSPACE</span><h1>{model?.name || '未命名设计'} · 装配</h1><p>{instanceCount} 个实例 · {envelope ? '当前模型为固定基准' : '尚未创建主件，可先添加标准件'} · 长度单位 mm</p></div><div className="aw-actions"><button onClick={onBackToModel}>返回 3D 建模</button><button onClick={onOpenLibrary}>＋ 添加标准件</button><button className="aw-primary" onClick={runCheck} disabled={!instanceCount}>运行预检查</button></div></header>
    ...
    </main><aside className="aw-card aw-inspector"><h2>装配树 <span>{instanceCount} 实例</span></h2>{envelope && <div className="aw-current-model"><span>◇</span><div><b>{model?.name || '当前零件'}</b><small>固定 · 世界坐标基准</small></div></div>}<div className="aw-tree">{assemblyItems.map((item, index) => <button key={item.id} className={selectedId === item.id ? 'selected' : ''} aria-pressed={selectedId === item.id} onClick={() => setSelectedId(item.id)}><span>{String(index + 1).padStart(2, '0')}</span><div><b>{item.name}</b><small>{item.spec}</small></div></button>)}</div>{!assemblyItems.length && <p className="aw-muted">{envelope ? '当前仅有主件；插入标准件后可编辑其位置。' : '装配树为空；创建主件或插入标准件后会显示实际实例。'}</p>}
