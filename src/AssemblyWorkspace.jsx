...
function AssemblyViewport({ model, items, selectedId, exploded, section, sectionZ, resetNonce }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      renderer.domElement.setAttribute('aria-label', mainModelEnvelope(model) ? '装配三维预览：拖动旋转，滚轮缩放。当前模型显示包络，标准件显示简化几何。' : '装配三维预览：拖动旋转，滚轮缩放。已插入标准件显示简化几何，尚未创建主件。')
      ...
      if (envelope) {
        ...
        box.position.set((envelope.min.x + envelope.max.x) / 2, (envelope.min.y + envelope.max.y) / 2, (envelope.min.z + envelope.max.z) / 2)
...
function TransformEditor({ item, onApply, canAlign, onAlign }) {
  const editableVector = (vector) => Object.fromEntries(['x', 'y', 'z'].map((axis) => [axis, vector?.[axis] ?? '']))
  const [position, setPosition] = useState(() => editableVector(item.position))
  const [rotation, setRotation] = useState(() => editableVector(item.rotation))
  const [errors, setErrors] = useState([])
  useEffect(() => { setPosition(editableVector(item.position)); setRotation(editableVector(item.rotation)); setErrors([]) }, [item])
  const submit = (event) => {
...
export default function AssemblyWorkspace({ model, assemblyItems = [], setAssemblyItems, onBackToModel, onOpenLibrary, showToast }) {
  ...
  const envelope = mainModelEnvelope(model)
  const hasMainModel = Boolean(model?.kind || model?.recipeId)
  ...
  useEffect(() => { if (!assemblyItems.some((item) => item.id === selectedId)) setSelectedId(assemblyItems[0]?.id || '') }, [assemblyItems, selectedId])
  ...
  const runCheck = () => {
    if (!instanceCount) return showToast?.('当前装配为空，请先创建主件或插入标准件。', 'info')
    setReport(checkAssembly(model, assemblyItems))
  }
  const errors = report?.issues.filter((issue) => issue.severity === 'error') || []
  const warnings = report?.issues.filter((issue) => issue.severity === 'warning') || []
  return <div className="aw-workspace">
    <header className="aw-heading"><div><span className="aw-eyebrow">ASSEMBLY WORKSPACE</span><h1>{model?.name || '未命名设计'} · 标准件装配草稿</h1><p>{instanceCount} 个实例 · {envelope ? '当前模型为固定基准' : hasMainModel ? '主件尺寸待更新，可继续编辑标准件' : '尚未创建主件，可先添加标准件'} · 长度单位 mm</p></div><div className="aw-actions"><button onClick={onBackToModel}>返回 3D 建模</button><button onClick={onOpenLibrary}>＋ 添加标准件</button><button className="aw-primary" onClick={runCheck} disabled={!instanceCount}>运行预检查</button></div></header>
    {hasMainModel && !envelope && <p className="aw-warning" role="status">当前主件尚无有效的实体包络。请返回 3D 建模检查并更新预览；标准件的位置仍会保存。</p>}
    <div className="aw-assembly-layout"><section className="aw-card aw-view-card"><div className="aw-view-toolbar"><span>装配空间</span><div className="aw-actions"><button aria-pressed={exploded} className={exploded ? 'selected' : ''} onClick={() => { setExploded((value) => !value); setResetNonce((n) => n + 1) }}>爆炸视图</button><button aria-pressed={section} className={section ? 'selected' : ''} onClick={() => setSection((value) => !value)}>剖切 Z</button><button onClick={() => setResetNonce((value) => value + 1)}>适合窗口</button></div></div>
      ...
      <div className="aw-view-legend">{envelope && <span><i className="aw-envelope-key" /> 当前模型包络</span>}<span><i className="aw-solid-key" /> 标准件简化几何</span><span>X 红 · Y 绿 · Z 蓝</span></div><p className="aw-scope">拖动旋转，滚轮缩放。{exploded ? '爆炸位移仅影响显示，保存位置和预检查仍使用装配坐标。' : envelope ? '半透明框是主件外形包络，未显示其孔槽或内部结构。' : '当前没有主件；可从标准件库开始建立装配。'}{section && '剖切只改变显示，不改变零件。'}</p>
      ...
      <section className="aw-check-panel" aria-live="polite"><div className="aw-section-heading"><h2>装配预检查</h2>{report && <button onClick={runCheck}>{stale ? '重新检查' : '重新运行'}</button>}</div>{!report ? <p className="aw-muted">{!instanceCount ? '装配为空，请先创建主件或插入标准件。' : envelope ? '点击“运行预检查”，按已保存的位置计算包络重叠与可判定的轴孔间隙。' : '点击“运行预检查”，检查已插入实例的数据与相互包络重叠；主件配合待主件创建后检查。'}</p> : stale ? <div className="aw-warning" role="status">模型或实例已变化，上一份结果已失效。请重新运行预检查。</div> : <><div className={`aw-check-summary ${errors.length ? 'error' : warnings.length ? 'warning' : ''}`}>{errors.length ? `发现 ${errors.length} 项尺寸 / 数据问题` : warnings.length ? `发现 ${warnings.length} 处包络重叠待复核` : report.instanceCount < 2 ? '实例数据检查通过；暂无其他实例可比较' : '本次未发现包络重叠或可判定的尺寸冲突'}<small>该结论不代表实体无干涉或装配可制造。</small></div>{report.issues.length > 0 && <ul className="aw-issues">{report.issues.map((issue, index) => <li className={issue.severity} key={`${issue.code}-${index}`}>{issue.message}</li>)}</ul>}{report.fits.length > 0 && <div className="aw-fit-list">{report.fits.map((fit) => { const item = assemblyItems.find((entry) => entry.id === fit.itemId); return <div key={fit.itemId}><b>{item?.name} · {item?.spec}</b><span>{fitLabels[fit.status]}</span><small>轴线偏移 {fit.axisOffset} mm · 公称径向间隙 {fit.radialClearance} mm</small></div> })}</div>}<p className="aw-scope">检查范围：{report.scope}</p></>}</section>
    </section><aside className="aw-card aw-inspector"><h2>装配树 <span>{instanceCount} 实例</span></h2>{hasMainModel && <div className="aw-current-model"><span>◇</span><div><b>{model?.name || '当前零件'}</b><small>{envelope ? '固定 · 世界坐标基准' : '主件尺寸待更新'}</small></div></div>}<div className="aw-tree">{assemblyItems.map((item, index) => <button key={item.id} className={selectedId === item.id ? 'selected' : ''} aria-pressed={selectedId === item.id} onClick={() => setSelectedId(item.id)}><span>{String(index + 1).padStart(2, '0')}</span><div><b>{item.name}</b><small>{item.spec}</small></div></button>)}</div>{!assemblyItems.length && <p className="aw-muted">{envelope ? '当前仅有主件；插入标准件后可编辑其位置。' : '装配树为空；创建主件或插入标准件后会显示实际实例。'}</p>}
      {selected && <><div className="aw-instance-spec"><b>{selected.spec}</b><small>{selected.source}</small><small>实例 {String(selected.id).slice(0, 8)} · 独立位置</small></div><TransformEditor key={selected.id} item={selected} onApply={replaceItem} canAlign={Boolean(envelope?.axial)} onAlign={() => replaceItem(applyPartTransform(selected, { x: envelope.length / 2, y: 0, z: 0 }, { x: 0, y: 0, z: 0 }))} /><div className="aw-instance-actions"><button disabled={Boolean(validatePartDefinition(selected).length || validateTransform(selected.position, selected.rotation).length)} onClick={() => { const copy = duplicatePartInstance(selected); setAssemblyItems((items) => [...items, copy]); setSelectedId(copy.id); setResetNonce((value) => value + 1) }}>复制实例</button><button className="aw-remove" onClick={removeItem}>移除实例</button></div></>}
