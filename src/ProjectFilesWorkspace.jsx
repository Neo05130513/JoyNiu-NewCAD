...
export default function ProjectFilesWorkspace({
  ...
  return <div className="pf-workspace">
    ...
    {documentFile && <section className="pf-document-card" aria-label="文档编辑器"><div className="pf-section-heading"><div><h2>{documentFile.name}</h2><p>正文随输入保存到此文件的工作草稿；保存版本后可回看。</p></div><button type="button" className="pf-button" onClick={() => setDocumentFileId(null)}>收起编辑器</button></div><label className="pf-document-label">文档正文<textarea value={documentFile.snapshot.documentText || ''} onChange={(event) => onUpdateDocument?.(documentFile.id, event.target.value, documentFile.projectId)} placeholder="记录设计要求、尺寸说明或交付备注…" /></label><div className="pf-heading-actions"><button type="button" className="pf-button" onClick={() => onDownloadFile?.(documentFile)}>下载文档</button><button type="button" className="pf-button pf-primary" onClick={() => openDialog('saveVersion', documentFile)}>保存文档版本</button></div></section>}
